...
#Q2.3  python ok -q check_city_abstraction
...
def berry_finder(t):
    """Returns True if t(tree) contains a node with the value 'berry' and 
    False otherwise.
    ...
    if label(t) == 'berry':
        return True
    for tree1 in branches(t):
        if berry_finder(tree1):
            return True
    return False
...
def sprout_leaves(t, leaves):
    ...
    """
    if is_leaf(t):
        leave = [0 for x in range(len(leaves))]
        for x in range(len(leaves)):

            leave[x] = tree(leaves[x])
        return tree(label(t), leave)
    return tree(label(t),
                [sprout_leaves(tree, leaves) for tree in branches(t)])
...
def riffle(deck):
    ...
    def return_index(x):
        if x % 2 == 0:
            return x // 2
        return (len(deck) // 2 + x // 2)

    return [deck[return_index(x)] for x in range(len(deck))]
